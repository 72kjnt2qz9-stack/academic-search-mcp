...
import { SearchParameters, Paper, Citation, AccessStatus } from '../types/index.js';
...
export class JstorInterface {
  ...
  private readonly minRequestInterval: number = 2000; // 2 seconds between requests (more conservative than Google Scholar)
  private readonly maxRetries: number = 3;
  private readonly baseBackoffDelay: number = 2000; // 2 second base delay
   ...
   */
  async performRequest(url: string, config?: AxiosRequestConfig): Promise<string> {
   ...
   */
  async parseSearchResults(html: string, fetchAbstracts: boolean = false, fetchFullText: boolean = false): Promise<Paper[]> {
    ...
    if (!resultElements || resultElements.length === 0) {
      ...
      if (articleLinks.length > 0) {
        ...
        articleLinks.each((index, element) => {
          if (papers.length >= 20) return false; // Limit results
          ...
          if (title && url && title.length > 10) { // Filter out navigation links
            papers.push({
              ...
              abstract: undefined,
              fullText: undefined,
              accessStatus: 'unknown' as AccessStatus
            });
          }
        });
      }
   ...
   */
  async extractAbstract(paperUrl: string): Promise<string | null> {
   ...
   * Will be implemented when authentication is added
   */
  async attemptFullTextAccess(paperUrl: string): Promise<{ fullText: string | null; accessStatus: AccessStatus }> {
   ...
   */
  async authenticateWithBrowser(jstorUrl?: string): Promise<{
    success: boolean;
    message: string;
    cookiesFound?: number;
