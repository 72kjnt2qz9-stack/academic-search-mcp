...
export class MCPServer {
     ...
     */
    async handleSearchTool(args) {
        try {
            // Validate input parameters
            if (!isValidSearchParameters(args)) {
                return this.createErrorResponse('INVALID_PARAMETERS', 'Invalid search parameters. Keywords array is required and must contain non-empty strings.', args);
            }
            ...
            if (searchParams.dateRange) {
                ...
                if (!dateValidation.valid) {
                    return this.createErrorResponse('INVALID_DATE_RANGE', dateValidation.message, searchParams.dateRange);
                }
            }
            ...
            if ('error' in result && result.error) {
                return this.createErrorResponse(result.code, result.message, result.details);
            }
            ...
            return this.createErrorResponse('INTERNAL_ERROR', `Internal server error: ${error instanceof Error ? error.message : 'Unknown error'}`, error);
        }
    }
    /**
     * Define the JSTOR search tool schema
     */
    getJstorSearchToolDefinition() {
        return {
            name: 'search_jstor',
     ...
     */
    async handleJstorAuthTool(args) {
        try {
            ...
            switch (action) {
                ...
                default:
                    return this.createErrorResponse('INVALID_ACTION', `Invalid action: ${action}. Use 'authenticate', 'status', or 'clear'`, { action, validActions: ['authenticate', 'status', 'clear'] });
            }
        }
        catch (error) {
            return this.createErrorResponse('AUTHENTICATION_ERROR', `JSTOR authentication failed: ${error instanceof Error ? error.message : 'Unknown error'}`, error);
        }
    }
    async handleJstorSearchTool(args) {
        try {
            // Validate input parameters
            if (!isValidSearchParameters(args)) {
                return this.createErrorResponse('INVALID_PARAMETERS', 'Invalid search parameters. Keywords array is required and must contain non-empty strings.', args);
            ...
            return this.createErrorResponse('INTERNAL_ERROR', `JSTOR search error: ${error instanceof Error ? error.message : 'Unknown error'}`, error);
        }
    }
